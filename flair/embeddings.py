...
class MuseCrosslingualEmbeddings(TokenEmbeddings):
    def __init__(self,
        ...
        :param language_code: if None, language will be detected for each sentence individually.
        If not None, a single-language for the while data will be assumed. This modification done to skip redundant calls for
        get_language_code() for each sentence.
        """
        self.name: str = f"muse-crosslingual"
        self.static_embeddings = True
        self.__embedding_length: int = 300
        self.language_embeddings = {}
        self.supported = ["en","de","bg","ca","hr","cs","da","nl","et","fi","fr","el","he","hu","id","it","mk","no","pl","pt","ro","ru","sk"]

        # assign language code common to the input data if that is the case.
        if language_code is not None:
            if language_code not in self.supported:
    ...
    def load_language_model(self, language_code: str = None):

        if language_code not in self.supported: language_code = "en"
