        ...
        for sentence in sentences:
            ...
            for token in sentence.tokens:
                ...
                subtoken_embeddings = _extract_embeddings(
                    ...
                    subword_end_idx=offset + num_subwords,
...
class BertEmbeddings(TokenEmbeddings):
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        ...
        ) # list of BertInputFeatures. dim = (len(sentences),1)
        all_input_ids = torch.LongTensor([f.input_ids for f in features]).to(
            flair.device
        ) #2 dim vector (num sentences, max seq len)
        ...
        self.model.to(flair.device)
        self.model.eval()
        all_encoder_layers = self.model(all_input_ids, attention_mask=all_input_masks)[ # Equivalent in hugginggace: Transformers>DistilbertForSequenceClassif>forward>distilbert_output.
            -1
        ] # all_hidden_states (since output_hidden_states = True above as hard-coded. ) (num_layers, bs, seq_len, dim)
        ...
        with torch.no_grad():
            ...
            for sentence_index, sentence in enumerate(sentences):
                ...
                subtoken_embeddings = []
                for token_index, _ in enumerate(feature.tokens): # here token_idx stands for the "token_index"th subtoken (wordpiece) of sentence, generated by BertTokenizer.
                    all_layers = [] #keeps layer outputs of the subtoken. (len(layer_indexes), dim)
                    for layer_index in self.layer_indexes:
                        ...
                        all_layers.append(layer_output[token_index]) #layer_output: (seq_len.dim) # layer_output[token_idx]: (dim)
                    ...
                    # eg. for the case of 2 subtokens in total, and 2 layers, output:[[layer 1 embedding of subtoken 1],[layer 2 embedding of subtoken 1],[lay 1 sub 2],[lay 2 syb 2]]

                # get the current sentence object
                token_idx = 0
                for token in sentence: #here token is the original token (composition of its subtokens)
...
class DocumentPoolEmbeddings(DocumentEmbeddings):
    ...
    def embed(self, sentences: Union[List[Sentence], Sentence]):
        ...
        self.embeddings.embed(sentences) #embeds tokens of sentences.
        ...
        for sentence in sentences:
            ...
            word_embeddings = torch.cat(word_embeddings, dim=0).to(flair.device) # (seq_len, dim)
